...
namespace Facebook_Tool_Request.Common
{
    internal class MinProxy
    {
        ...
        public MinProxy(string api_key, int typeProxy, int limit_theads_use = 1, string location = "")
        {
        ...
        public bool ChangeProxy()
        {
            ...
            ip = "";
            port = 0;
            string text = RequestGet("https://dash.minproxy.vn/api/rotating/v1/proxy/get-new-proxy?api_key=" + api_key);
            if (text != "")
            {
                try
                {
                    ...
                    else if (jObject["code"].ToString() == "1")
                    {
                        if (typeProxy == 0)
                        {
                            proxy = jObject["data"]["http_proxy"].ToString();
        ...
        public bool CheckStatusProxy()
        {
            ...
            ip = "";
            port = 0;
            string text = RequestGet("https://dash.minproxy.vn/api/rotating/v1/proxy/get-current-proxy?api_key=" + api_key);
            if (text != "")
            {
                try
                {
